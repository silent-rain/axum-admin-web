...
impl AppTemplateService {
    ...
    /// 获取{{InterfaceName}}详情
    pub async fn info(&self, req: GetAppTemplateReq) -> Result<template::Model, ErrorMsg> {
        let result = self
            .app_template_dao
            .info(req.id)
            .await
    ...
    /// 添加{{InterfaceName}}
    pub async fn create(&self, data: CreateAppTemplateReq) -> Result<template::Model, ErrorMsg> {
