...
use crate::{
    dao::t_app_template::TemplateDao,
    dto::t_app_template::{
...
impl TemplateService {
    ...
    pub async fn list(
        &self,
        req: GetTemplatesReq,
    ) -> Result<(Vec<t_app_template::Model>, u64), ErrorMsg> {
        if req.is_all {
    ...
    /// 获取{{InterfaceName}}详情
    pub async fn info(&self, req: GetTemplateReq) -> Result<t_app_template::Model, ErrorMsg> {
        let result = self
            .template_dao
            .info(req.id)
            .await
    ...
    /// 添加{{InterfaceName}}
    pub async fn create(&self, data: CreateTemplateReq) -> Result<t_app_template::Model, ErrorMsg> {
        let model = t_app_template::ActiveModel {
    ...
    ) -> Result<t_app_template::Model, ErrorMsg> {
        let result = self
            .template_dao
