...
pub async fn check_captcha(
    ...
    let result = captcha_dao
        ...
        .map_err(|err| {
            ...
            Error::DbQueryError.into_err_with_msg("查询验证码失败")
        })?
        .ok_or_else(|| {
            error!("验证码信息不存在, captcha: {}", captcha);
            Error::DbQueryEmptyError.into_err_with_msg("验证码信息不存在")
        })?;
    ...
    if !result.status {
        return {
            ...
            Err(Error::CaptchaInvalid.into_err_with_msg("验证码已失效"))
        };
    }
    ...
    if result.captcha.to_uppercase() != captcha.to_uppercase() {
        return {
            ...
            Err(Error::CaptchaInvalid.into_err_with_msg("验证码错误"))
        };
    }
    ...
    if now > max_time {
        return {
            ...
            Err(Error::CaptchaExpire.into_err_with_msg("验证码过期"))
        };
    }
    ...
    captcha_dao
        ...
        .map_err(|err| {
            ...
            Error::DbQueryError.into_err_with_msg("设置验证码状态失败")
        })?;
